...
async def health_check_detailed() -> Dict[str, Any]:
    ...
    postgres_result, mongo_result, redis_result, chroma_result, mcp_result = await asyncio.gather(
        _check_postgres(),
        _check_mongodb(),
        _check_redis(),
        _check_chroma(),
    ...
    all_ok = all(
        r.get("status") == "ok"
        for r in [postgres_result, mongo_result, redis_result, chroma_result, mcp_result]
    )
